...
// ── Chromium — usar el binario del sistema (instalado en Dockerfile) ──────────
const browserExecutable = process.env.REMOTION_CHROME_EXECUTABLE || undefined;
console.log("Chromium executable:", browserExecutable || "auto-detect");
...
app.post("/render", async (req, res) => {
  ...
  setImmediate(async () => {
    try {
      ...
      console.log(`Job ${jobId} iniciando bundle...`);
      ...
      console.log(`Job ${jobId} renderizando ${Math.ceil(duration * fps)} frames...`);
      ...
      await renderMedia({
        ...
        onProgress: ({ progress }) => {
          ...
          if (jobs[jobId].progress % 10 === 0) {
            console.log(`Job ${jobId} progreso: ${jobs[jobId].progress}%`);
          }
        },
        chromiumOptions: { disableWebSecurity: true },
        timeoutInMilliseconds: 900000, // 15 min máximo
      });

      const baseUrl = process.env.BASE_URL || `http://localhost:${PORT}`;
      jobs[jobId].status = "done";
      jobs[jobId].url = `${baseUrl}/output/${jobId}.mp4`;
      console.log(`Job ${jobId} completado: ${jobs[jobId].url}`);
    } catch (err) {
      console.error(`Job ${jobId} falló:`, err);
